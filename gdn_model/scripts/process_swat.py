...
def norm(train, test):
    ...
    # mylibrary.save_variable(normalizer, 'swat_scaler')
...
def main():
    ...
    test = pd.read_csv('./data/swat/all_attack_id/attack_various_label.csv', index_col=0)
    train = pd.read_csv('./data/swat/all_attack_id/swat_train.csv', index_col=0)
    ...
    # normalizer = mylibrary.load_variable('swat_scaler')
    # print(normalizer.inverse_transform(x_test[0, :].reshape(1, -1)))
    # raise Exception()
    ...
    train_df.to_csv('./data/swat/all_attack_id/train.csv')
    test_df.to_csv('./data/swat/all_attack_id/test.csv')
